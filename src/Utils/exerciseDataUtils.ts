...
export const saveExercisesForDay = (
   date: Date,
...
const updateExerciseNamesList = (exercises: IExerciseEntry[]) => {
   ...
   const standardExercisesLower = Object.values(POPULAR_EXERCISES)
      .reduce<string[]>((all, list) => [...all, ...list], [])
      .map((name) => name.toLowerCase());
   POPULAR_EXERCISES;
   const newNamesFromEntries = exercises
      .map((ex) => ex.name.trim())
      .filter(
         (name) => name && !standardExercisesLower.includes(name.toLowerCase())
      );
...
export const updateExerciseName = (
   ...
   const index = exerciseNames.findIndex(
      (name) => name.toLowerCase() === oldNameLower
   );
...
export const removeExerciseName = (exerciseName: string): boolean => {
   ...
   const updatedNames = exerciseNames.filter(
      (name) => name.toLowerCase() !== nameToRemoveLower
   );
